...
def prepare_cmips_data(cfg):
    ...
    with tf.device('/cpu:0'):
        gen_model = keras.models.load_model('convlstm-cmip')
    ...
    dict_cmips = []
    for i in range(32):
        one_year_sst = cmip_data['sst'][start:end, 0:12, :, :]
        # (samples, time, rows, cols, channels)
        gen_output = gen_model(one_year_sst)
        breakpoint()
        next_two_year_nino = cmip_label['nino'][start:end, 12:24]
        dict_cmip = prepare_cmip_data(cfg, {'sst': one_year_sst},
                                      next_two_year_nino)
...
def build_cmip_dataset(cfg, dict_cmip):
    ...
    if cfg.DEBUG:
        fold_num = 2
        dataset0, dataset1, dataset2, dataset3, dataset4, dataset_other = data.random_split(
            dataset_cmip, lengths=[fold_num, fold_num, fold_num, fold_num, fold_num, len(dataset_cmip) - 5 * fold_num],
...
def build_soda_dataset(cfg):
    ...
    if cfg.DEBUG:
        fold_num = 2
        dataset0, dataset1, dataset2, dataset3, dataset4, dataset_other = data.random_split(
            dataset_soda, lengths=[fold_num, fold_num, fold_num, fold_num, fold_num, dataset_soda.len - 5 * fold_num],
            generator=torch.Generator().manual_seed(cfg.SEED))
    else:
